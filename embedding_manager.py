...
class EmbeddingManager:
    ...
    def create_embedding(self, text: str) -> np.ndarray:
        """
        Create an embedding for the given text using Google's Generative AI.
    ...
    def save_embedding(
        ...
        self.vectorstore.add_documents([doc], ids=[embedding_id])

        return embedding_id

    # def get_embedding_from_id(
    #     self, embedding_id: str
    # ) -> Optional[tuple[str, np.ndarray, Dict[str, Any]]]:
    #     """
    #     Retrieve a text and its embedding from ChromaDB.

    #     Args:
    #         embedding_id (str): The ID of the embedding to retrieve

    #     Returns:
    #         Optional[tuple[str, np.ndarray, Dict[str, Any]]]: A tuple of (text, embedding, metadata) if found, None otherwise
    #     """
    #     try:
    #         # Get document from ChromaDB
    #         doc = self.vectorstore.get(ids=[embedding_id])
    #         if not doc:
    #             return None

    #         # Get the embedding
    #         embedding = self.create_embedding(doc[0].page_content)

    #         return (doc[0].page_content, embedding, doc[0].metadata)
    #     except Exception:
    #         return None
    ...
    #     Args:
    #         embedding_id (str): The ID of the embedding to delete
    ...
    # ) -> bool:
    #     """
    #     Update an existing embedding's text or metadata.

    #     Args:
    #         embedding_id (str): The ID of the embedding to update
    #         text (str, optional): New text to update
    #         metadata (Dict[str, Any], optional): New metadata to update
    ...
    #         existing = self.get_embedding_from_id(embedding_id)
    #         if existing is None:
    #             return False

    #         current_text, current_embedding, current_metadata = existing
    ...
    #         # Add updated entry
    #         self.save_embedding(
    #             text=new_text, metadata=new_metadata, embedding=current_embedding
    #         )
